...
$script:Strings = @{
  en = @{
    ...
    m_op9 = "9 - Check + apply OS tweaks"
    m_op9_apply = "Apply OS tweaks now? (Y/N, Enter = N)"
    m_op0 = "0 - Exit"
    m_prompt = "Choose [0-9]"
    m_invalida = "Invalid option. Type a number from 0 to 9."
    ...
    m_opt4_prompt_seg = "Duration of each phase in seconds (Enter = 180)"
    ...
    m_opt4_prompt_modo = "Load mode: All (full), Single (1 core), Dual (2 cores) or Half (half chip)? (Enter = All)"
    m_opt4_prompt_grupo = "Core group (e.g.: 0,1 or all-grupos to sweep, Enter = none)"
    ...
    c_titulo = "=== OS check (read-only, no changes) ==="
    c_plano = "Active power plan: {0}"
    c_plano_tip = "Tip: Balanced, High Performance or Bitsum HP all work; avoid Power Saver."
    c_park = "Core parking: {0}"
    c_park_tip = "Tip: 100% minimum cores (ParkControl) avoids unpark latency."
    c_timer = "Current timer: {0} ms (min {1} / max {2})"
    c_timer_tip = "Tip: 0.5 ms via ISLC/TimerTool helps 1% lows; Windows default is ~15.6 ms."
    c_standby = "Standby cache: {0} | free: {1} (total {2})"
    c_standby_tip = "Tip: clear the standby list with ISLC if it stutters after long idle."
    c_tools = "Timer tools detected: {0}"
    c_tools_none = "none"
    c_tools_tip = "Tip: run only ONE timer tool at a time to avoid conflicts."
    c_sem_dados = "no data (details may need admin)"
    c_ok = "OK"
    c_aviso = "WARNING"
    ...
    g_pausa = "Press ENTER to continue..."
    ...
    a_os_titulo = "=== Apply OS tweaks (power plan + unpark) ==="
    a_os_admin = "Admin required: confirm the UAC prompt (or run the menu as administrator), then retry."
    a_os_plano_mantem = "Power plan already High performance or better ({0}): keeping it."
    a_os_plano_troca = "Setting High performance plan (powercfg /setactive SCHEME_MIN)..."
    a_os_unpark = "Setting unpark 100% on AC+DC (CPMINCORES + PROCTHROTTLEMIN)..."
    a_os_ok = "Done: plan + unpark applied (idempotent - safe to run again)."
    a_os_whatif = "WhatIf: would run: {0}"
    a_os_nada = "Dry-run only: nothing was changed."
    a_os_timer = "Timer: Windows cannot persist 0.5 ms alone - start ISLC.exe and enable 0.5 ms timer + standby purge."
    a_os_reverte = "Revert: powercfg /setactive SCHEME_BALANCED + unpark back with CPMINCORES 0 (see docs/ferramentas-extras.md)."
    p_titulo = "=== PBO limits (PPT/TDC/EDC, read-only) ==="
    p_sem_exe = "ryzen-smu-cli not found at {0}. Run menu option 1 first."
    p_nao_suportado = "SMU limits not supported in this ryzen-smu-cli version (--get-pbo-limits unknown)."
    p_fallback = "Fallback: current PPT via sensors; exact PPT/TDC/EDC limits in HWiNFO64."
    g_triagem_l1 = "NOTE: this kit test is a minutes-long screening - PASS does not guarantee stability in games/light use (idle, FPS-capped)."
    g_triagem_l2 = "If you crash/WHEA/BSOD in real use, back off 5 points (e.g. -30 to -25, or +5 on the failing core) and validate days of real use."
    t_modo_invalido = "ERROR: invalid load mode '{0}' (use All, Single, Dual or Half)."
    t_grupo_invalido = "ERROR: invalid core group '{0}' (physical cores 0 to {1})."
    t_carga_linha = "Load mode {0} ({1} threads{2})"
    t_affinity_fallback = "WARNING: affinity failed, running without affinity (approximate load)."
    t_ranking_cab = "=== ranking by group (higher effective clock first) ==="
    t_ranking_linha = "GROUP {0}: effAvg={1}MHz stretch={2}% WHEA={3}"
    t_ranking_linha_v = "GROUP {0}: effAvg={1}MHz stretch={2}% WHEA={3} [{4}]"
    t_ranking_linha_p = "GROUP {0}: effAvg={1}MHz stretch={2} WHEA={3} [{4}]"
    t_stretch_parcial = "N/A (partial load)"
    t_ranking_arquivo = "Ranking saved to: {0}"
    t_ranking_erro = "WARNING: ranking file write failed ({0}); continuing without ranking file."
    m_opt4_dica = "Tip: Single|Dual|Half = capped-FPS gaming (few cores, high boost); All = compile bursts (~100% all-core). Test gaming first, then All."
    t_rank_titulo = "# Core-group ranking - {0}"
    t_rank_cpu = "CPU: {0} ({1} physical / {2} logical)"
    t_rank_params = "Tested OffsetB: {0} | Load: {1}"
    t_rank_tab_cab = "| Group | effAvg (MHz) | stretch | WHEA | Verdict |"
    t_rank_rec_melhor = "Best group {0} (effAvg {1}MHz): candidate for aggressive per-core CO via menu option 3."
    t_rank_rec_ruim = "Group {0}: FAIL/WHEA ({1}) - back off 5 points on these cores."
    t_rank_sug_csv = "SUGESTAO: {0}"
    t_rank_sug_como = "Apply via menu option 3 (paste the CSV above)."
  }
  pt = @{
    m_titulo = "=== Ryzen CO Toolkit - Menu ==="
    ...
    m_op8 = "8 - Language/Idioma (EN/PT)"
    m_op9 = "9 - Checar + aplicar ajustes de SO"
    m_op9_apply = "Aplicar ajustes de SO agora? (S/N, Enter = N)"
    m_op0 = "0 - Sair"
    m_prompt = "Escolha [0-9]"
    m_invalida = "Opcao invalida. Digite um numero de 0 a 9."
    ...
    m_opt4_prompt_seg = "Duracao de cada fase em segundos (Enter = 180)"
    ...
    m_opt4_prompt_modo = "Modo de carga: All (tudo), Single (1 nucleo), Dual (2 nucleos) ou Half (metade)? (Enter = All)"
    m_opt4_prompt_grupo = "Grupo de nucleos (ex.: 0,1 ou all-grupos p/ varrer, Enter = nenhum)"
    ...
    c_titulo = "=== Check de SO (somente leitura, sem mudancas) ==="
    c_plano = "Plano de energia ativo: {0}"
    c_plano_tip = "Dica: Equilibrado, Alto Desempenho ou Bitsum HP servem; evite Economia de Energia."
    c_park = "Core parking: {0}"
    c_park_tip = "Dica: 100% de nucleos minimos (ParkControl) evita latencia ao desestacionar."
    c_timer = "Timer atual: {0} ms (min {1} / max {2})"
    c_timer_tip = "Dica: 0.5 ms via ISLC/TimerTool ajuda os 1% lows; o padrao do Windows e ~15.6 ms."
    c_standby = "Cache standby: {0} | livre: {1} (total {2})"
    c_standby_tip = "Dica: limpe a standby list com o ISLC se travar apos idle longo."
    c_tools = "Ferramentas de timer detectadas: {0}"
    c_tools_none = "nenhuma"
    c_tools_tip = "Dica: rode UMA ferramenta de timer por vez para evitar conflito."
    c_sem_dados = "sem dados (detalhe pode precisar de admin)"
    c_ok = "OK"
    c_aviso = "AVISO"
    ...
    g_pausa = "Pressione ENTER para continuar..."
    ...
    a_os_titulo = "=== Aplicar ajustes de SO (plano + unpark) ==="
    a_os_admin = "Admin necessario: confirme o UAC (ou rode o menu como administrador) e tente de novo."
    a_os_plano_mantem = "Plano ja e Alto desempenho ou melhor ({0}): mantendo."
    a_os_plano_troca = "Mudando para Alto desempenho (powercfg /setactive SCHEME_MIN)..."
    a_os_unpark = "Ajustando unpark 100% em AC+DC (CPMINCORES + PROCTHROTTLEMIN)..."
    a_os_ok = "Pronto: plano + unpark aplicados (idempotente - pode rodar de novo)."
    a_os_whatif = "WhatIf: executaria: {0}"
    a_os_nada = "Dry-run apenas: nada foi alterado."
    a_os_timer = "Timer: o Windows nao fixa 0.5 ms sozinho - abra o ISLC.exe e ative timer 0.5 ms + limpeza standby."
    a_os_reverte = "Reversao: powercfg /setactive SCHEME_BALANCED + unpark de volta com CPMINCORES 0 (ver docs/ferramentas-extras.md)."
    p_titulo = "=== Limites PBO (PPT/TDC/EDC, somente leitura) ==="
    p_sem_exe = "ryzen-smu-cli nao encontrado em {0}. Rode a opcao 1 do menu primeiro."
    p_nao_suportado = "Limites via SMU nao suportados nesta versao do ryzen-smu-cli (--get-pbo-limits desconhecido)."
    p_fallback = "Alternativa: PPT atual via sensores; limites exatos de PPT/TDC/EDC no HWiNFO64."
    g_triagem_l1 = "AVISO: este teste do kit e so uma triagem de minutos - PASSAR nao garante estabilidade em jogos/uso leve (idle, FPS capado)."
    g_triagem_l2 = "Se crashar/WHEA/BSOD no uso real, recue 5 pontos (ex.: -30 para -25, ou +5 no nucleo culpado) e valide dias de uso real."
    t_modo_invalido = "ERRO: modo de carga invalido '{0}' (use All, Single, Dual ou Half)."
    t_grupo_invalido = "ERRO: grupo de nucleos invalido '{0}' (nucleos fisicos 0 a {1})."
    t_carga_linha = "Modo de carga {0} ({1} threads{2})"
    t_affinity_fallback = "AVISO: affinity falhou, rodando sem affinity (carga aproximada)."
    t_ranking_cab = "=== ranking por grupo (maior clock efetivo primeiro) ==="
    t_ranking_linha = "GRUPO {0}: effMed={1}MHz stretch={2}% WHEA={3}"
    t_ranking_linha_v = "GRUPO {0}: effMed={1}MHz stretch={2}% WHEA={3} [{4}]"
    t_ranking_linha_p = "GRUPO {0}: effMed={1}MHz stretch={2} WHEA={3} [{4}]"
    t_stretch_parcial = "N/A (carga parcial)"
    t_ranking_arquivo = "Ranking salvo em: {0}"
    t_ranking_erro = "AVISO: falha ao gravar ranking ({0}); seguindo sem arquivo de ranking."
    m_opt4_dica = "Dica: Single|Dual|Half = jogo com FPS capado (poucos nucleos, boost alto); All = compilacao (~100% all-core). Teste gaming primeiro, depois All."
    t_rank_titulo = "# Ranking por grupo de nucleos - {0}"
    t_rank_cpu = "CPU: {0} ({1} fisicos / {2} logicos)"
    t_rank_params = "OffsetB testado: {0} | Carga: {1}"
    t_rank_tab_cab = "| Grupo | effMed (MHz) | stretch | WHEA | Veredito |"
    t_rank_rec_melhor = "Melhor grupo {0} (effMed {1}MHz): candidato a CO agressivo por nucleo via opcao 3."
    t_rank_rec_ruim = "Grupo {0}: FAIL/WHEA ({1}) - recue 5 pontos nestes nucleos."
    t_rank_sug_csv = "SUGESTAO: {0}"
    t_rank_sug_como = "Aplique via opcao 3 do menu (cole o CSV acima)."
  }
}
